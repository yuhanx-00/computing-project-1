...
ggplot(summary_percent_cocoa, aes(x = cocoa_percent_category , y = rating)) + 
  ...
  labs(x = "chocolate percentage category", y = "ratings", title = "Ratings by Chocolate Percentage Category in Each Country") +
  theme(
...
ggplot(chocolate_w_continent, aes(x = continent, y = rating)) + 
  ...
  labs(x = "continents", y = "ratings", title = "Ratings by Continent") +
  theme(
    plot.title = element_text(hjust = 0.5)  # Center the title
  )
```
...
# create vectors to store ingredients of interest
ingredients_names <- c("beans", "sugar", "cocoa_butter", "vanilla", "lecithin", "salt")
ingredients_abbr <- c("B", "S", "C", "V", "L", "Sa")

if (length(ingredients_names) != length(ingredients_abbr)) {
  stop("ingredients_names has a different length from ingredients_abbr.")
}

# add new columns for indicating if an ingredient exists
for (i in 1:length(ingredients_abbr)) {
    chocolate_feature <- chocolate_feature %>% 
      mutate(!!as.name(ingredients_names[i]) := case_when( 
          str_detect(ingredients, as.character(ingredients_abbr[i])) == TRUE ~ 1, 
          TRUE ~ 0
      ))
}
```
...
# create vectors to store memorable characteristics of interest
...
if (length(char_names) != length(char_abbr)) {
  stop("char_names has a different length from char_abbr.")
}

# add new columns for indicating if an ingredient exists
for (i in 1:length(char_abbr)) {
    chocolate_feature <- chocolate_feature %>% 
      mutate(!!as.name(char_names[i]) := case_when( 
          str_detect(most_memorable_characteristics, as.character(char_abbr[i])) == TRUE ~ 1, 
          TRUE ~ 0
      ))
}
```
...
# create a function that takes in a dataframe and calculate the mean value of a column
summarize_code <- function(data, col) {
  col_name <- paste("means_", col, sep="")
  data %>%
    summarize(!!col_name := mean(!!sym(col)))
}
...
# create a list that stores two types of columns (ingredients and characteristics)
